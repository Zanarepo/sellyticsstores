      ...
      {stats.map((stat, index) => {
        ...
        return (
          <motion.div
            ...
            className={`bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm bg-gradient-to-br ${stat.gradient}`}
          >
            <div className="flex items-center gap-4">
              <div className={`w-14 h-14 rounded-2xl ${stat.bgIcon} flex items-center justify-center flex-shrink-0`}>
              ...
              <div>
                ...
                {stat.label === 'Monthly Coverage' && (
                  <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">
                    vs {daysInMonth.length} days × {totalStaff} staff
                  </p>
                )}
              </div>
            </div>
