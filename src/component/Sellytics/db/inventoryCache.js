...
export const fetchRestockHistory = async (productId, storeId) => {
  ...
  ...log,
  dynamic_product_name: log.dynamic_product?.name || '',
...
await db.product_inventory_adjustments_logs.bulkPut(flattenedLogs);
    ...
    // Return cached logs if offline or error
    return db.product_inventory_adjustments_logs
      .where('store_id')
      .equals(storeId)
      .and((log) => log.dynamic_product_id === productId)
