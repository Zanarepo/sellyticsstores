/**
 * SwiftCheckout - Device Line Handler Hook
 * Handles complex device ID validation and line management
 */
    ...
    if (field === 'deviceIds' && deviceIdx !== null) {
      ...
      if (isBlur && value.trim()) {
        ...
        const { data: productData, error } = await supabase
          ...
          .select('id, name, selling_price, dynamic_product_imeis, device_size, is_unique')
          .eq('store_id', storeId)
        ...
        setLines((ls) => {
          ...
          if (existingLineIdx !== -1 && existingLineIdx !== lineIdx) {
            ...
            next[lineIdx].is_unique = productData.is_unique || false;
      ...
      setLines((ls) => {
        ...
        if (field === 'deviceSizes' && deviceIdx !== null) {
          ...
          if (prod) {
            ...
            next[lineIdx].is_unique = prod.is_unique || false;
