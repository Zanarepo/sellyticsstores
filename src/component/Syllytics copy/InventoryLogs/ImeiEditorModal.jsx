 ...
 * Modal for managing IMEIs/device IDs for unique products
 */
...
export default function ImeiEditorModal({
  ...
  useEffect(() => {
    if (product?.dynamic_product_imeis) {
      const parsed = product.dynamic_product_imeis
        .split(',')
        .map(i => i.trim())
        .filter(Boolean);
      setImeis(parsed);
  ...
  const handleAddImei = async () => {
    ...
    if (imeis.map(i => i.toLowerCase()).includes(trimmed.toLowerCase())) {
      toast.error('This device ID already exists');
      return;
    }

    const success = await onAddImei(product.id, trimmed);
    if (success) {
      setImeis(prev => [...prev, trimmed]);
      setNewImei('');
  ...
  const handleRemoveImei = async (imei) => {
    ...
    if (success) {
      setImeis(prev => prev.filter(i => i !== imei));
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Tracked IDs</span>
                <span className="text-sm text-slate-500">{imeis.length} units</span>
              </div>
