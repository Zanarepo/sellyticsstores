...
,
  ...
  async updateInventoryFromImeis(productId, storeId, newCount, userEmail, reason) {
    const { data: inv, error: fetchErr } = await supabase
      .from('dynamic_inventory')
      .select('id, available_qty')
      .eq('dynamic_product_id', productId)
    ...
    const oldQty = inv.available_qty;
    const difference = newCount - oldQty;

    if (difference !== 0) {
      await supabase
        .from('dynamic_inventory')
        .update({ 
          available_qty: newCount,
          updated_at: new Date().toISOString()
        })
        .eq('id', inv.id);
  ...
  async checkPermissions(storeId, userEmail) {
    ...
    try {
      ...
      if (userData) {
        ...
        canAdjust = ['store', 'manager', 'admin', 'md'].includes(role);
