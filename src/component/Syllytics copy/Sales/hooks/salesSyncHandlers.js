...
// Create dynamic sale
export async function createDynamicSale(item, saleGroupMapRef) {
  const metadata = getCreatorMetadata();
  const { data, queue_id } = item;

  const onlineSaleGroupId = saleGroupMapRef.current[data.client_sale_group_ref];
  if (!onlineSaleGroupId) throw new Error(`Sale group not synced yet for offline ref ${data.client_sale_group_ref}`);
