...
export default function ReturnsByDeviceIdManager() {
  ...
  const { data: sales, error: salesError } = await supabase
    ...
    .eq('store_id', storeId)
    .or(`
      device_id.ilike.%${deviceIdQuery}%,
      dynamic_product_imeis.ilike.%${deviceIdQuery}%,
      device_id.is.null
    `);   // <-- THIS MAKES EMPTY DEVICES ALSO RETRIEVE
     ...
     const deviceIds = deviceIdQuery
  ? [deviceIdQuery]
  : (sale.device_id?.split(',').filter(id => id.trim()) || []);

const quantity = deviceIds && deviceIds.length > 0
  ? deviceIds.length
  : (receipt?.sales_qty || 0);  // <-- FIX
...
const totalAmount = unitPrice * quantity;
  ...
  const cleanedDeviceId = form.device_id?.trim() === "" ? null : form.device_id;

  const returnData = {
    receipt_id: parseInt(form.receipt_id),
    customer_address: form.customer_address,
    product_name: form.product_name,
    device_id: cleanedDeviceId,
  ...
  try {
    // 1️⃣ Save or update returns table
    let result;
    if (editing && editing.id) {
      result = await supabase.from("returns").update(returnData).eq("id", editing.id);
    } else {
      result = await supabase.from("returns").insert([returnData]).select().single();
    }
    ...
    // 2️⃣ GET the product from dynamic_product using the product_name
    const { data: product, error: productError } = await supabase
      .from("dynamic_product")
      .select("id, name, purchase_qty")
      .eq("name", form.product_name)
      .eq("store_id", storeId)
      .single();
    ...
    // Determine the qty to add:
// If device_id list is provided → each IMEI counts as 1 qty
// If NOT provided → use the manually entered qty
// 3️⃣ UPDATE or INSERT the product back into dynamic_product
let productId;
const qtyToSet = cleanedDeviceId 
  ? cleanedDeviceId.length
  : parseInt(form.qty);
...
if (product) {
  const { data: updatedProduct, error: updateError } = await supabase
    .from("dynamic_product")
    .update({
      purchase_qty: qtyToSet,
      dynamic_product_imeis: cleanedDeviceId || product.dynamic_product_imeis
        ? [...(product.dynamic_product_imeis || []), ...cleanedDeviceId]
        : null
    })
    .eq("id", product.id)
    .select()
    ...
    .insert([
      {
        ...
        purchase_qty: qtyToSet,
        dynamic_product_imeis: cleanedDeviceId ? cleanedDeviceId : null
      }
    ])
    .select()
    .single();
...
// 4️⃣ Ask for confirmation before updating inventory
const confirmInventory = window.confirm(
  "Do you want to return these items back to inventory? This will append the quantity to stock."
);
...
if (confirmInventory) {
  ...
  if (inventory) {
    await supabase
      .from("dynamic_inventory")
      .update({ available_qty: inventory.available_qty + qtyToSet })
      .eq("id", inventory.id);
  } else {
    await supabase
      .from("dynamic_inventory")
      .insert([
        {
          ...
          quantity: qtyToSet,
          available_qty: qtyToSet
        }
      ]);
  }

  toast.success("Returned items successfully appended to inventory!");
}
    ...
    setError(null);
    // re-fetch UI
    // (your existing RETURN table refresh logic here)
